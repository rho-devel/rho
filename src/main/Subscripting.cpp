...
void Subscripting::Indices::applyNewNames(VectorBase* v) const
{
	...
	else newnames = new StringVector(v->size());
    }
				       ...
				       const StringVector* range_names)
{
	...
	if (subscript == String::NA())
		...
		if (found) 
		    ...
		    if (!m_use_names)
			m_use_names = new ListVector(rawsize);
		    ++m_max_index;
			   ...
			   const StringVector* range_names)
{
	...
	GCStackRoot<RealVector> canvec(new RealVector(indices.size()));
	...
	GCStackRoot<IntVector> canvec(new IntVector(indices.size()));
	for (std::size_t i = 0; i < indices.size(); ++i) {
	    std::size_t index = indices[i];
...
bool Subscripting::dropDimensions(VectorBase* v)
{
	...
	{
	    GCStackRoot<IntVector> newdims(new IntVector(ngooddims));
	    std::size_t dout = 0;
	...
	if (havenames) {
	    ...
	    GCStackRoot<ListVector> newdimnames(new ListVector(ngooddims));
	    GCStackRoot<StringVector>
		newdimnamesnames(new StringVector(ngooddims));
	    std::size_t dout = 0;
	    for (std::size_t din = 0; din < ndims; ++din)
		if ((*dims)[din] != 1) {
				      ...
				      bool drop)
{
	...
	GCStackRoot<IntVector> newdims(new IntVector(ndims));
	for (std::size_t d = 0; d < ndims; ++d)
	...
	if (dimnames) {
	    GCStackRoot<ListVector> newdimnames(new ListVector(ndims));
	    for (std::size_t d = 0; d < ndims; ++d) {
