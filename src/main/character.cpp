...
#ifdef SUPPORT_MBCS
		buf = reinterpret_cast<char*>(R_AllocStringBuffer(slen+vlen,
								  &cbuff));
...
SEXP attribute_hidden do_gsub(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    if (length(rep) < 1) error(R_MSG_IA);
...
SEXP attribute_hidden do_agrep(SEXP call, SEXP op, SEXP args, SEXP env)
{
    ...
    aps = apse_create(reinterpret_cast<unsigned char *>(const_cast<char*>(str)),
		      apse_size_t(strlen(str)), max_distance_opt);
    if(!aps)
