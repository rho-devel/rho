...
SEXP attribute_hidden do_grep(SEXP call, SEXP op, SEXP args, SEXP env)
{
	...
	if (STRING_ELT(text, i) != NA_STRING) {
	    ...
	    if (fixed_opt)
		LOGICAL(ind)[i] = fgrep_one(spat, s, CXXRCONSTRUCT(Rboolean, useBytes), use_UTF8, NULL) >= 0;
	    else if (perl_opt) {
...
SEXP attribute_hidden do_regexpr(SEXP call, SEXP op, SEXP args, SEXP env)
{
	...
	for (i = 0 ; i < n ; i++) {
		...
		if (fixed_opt) {
		    int st = fgrep_one(spat, s, CXXRCONSTRUCT(Rboolean, useBytes), use_UTF8, NULL);
	...
	for (i = 0 ; i < n ; i++) {
		...
		if (fixed_opt || perl_opt) {
			...
			if (fixed_opt)
			    ...
			    elt = gregexpr_perl(spat, s, re_pcre, re_pe, CXXRCONSTRUCT(Rboolean, useBytes), use_UTF8);
		    }
		} else
